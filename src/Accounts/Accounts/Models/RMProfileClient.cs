...
namespace Microsoft.Azure.Commands.ResourceManager.Common
{
    public class RMProfileClient
    {
        ...
        public IList<IAzureSubscription> TryGetSubscriptionById(string tenantId, string subscriptionId)
        {
            Guid subscriptionIdGuid;
            IList<IAzureSubscription> subscriptions = new List<IAzureSubscription>();
            if (Guid.TryParse(subscriptionId, out subscriptionIdGuid))
            {
                var tenants = string.IsNullOrEmpty(tenantId) ? ListTenants(tenantId) :
  new List<AzureTenant>() { CreateTenant(tenantId) };
                ...
                foreach (var tenant in tenants)
                {
                    try
                    {
                        ...
                        var sub = SubscriptionAndTenantClient?.GetSubscriptionById(subscriptionId, accessToken, account, environment);
