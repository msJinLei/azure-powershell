...
namespace Microsoft.Azure.Commands.ResourceManager.Common
{
    public class RMProfileClient
    {
        ...
        public RMProfileClient(IProfileOperations profile)
        {
            ...
            clientSwitcher = new SubscriptionClientSwitcher();
        }
            ...
            bool shouldPopulateContextList = true)
        {
            ...
            if (skipValidation)
            {
                ...
                newSubscription.SetTenant(tenantId);
                newSubscription.SetAccount(account.Id);
            ...
            else
            {
                ...
                if (!string.IsNullOrEmpty(tenantId))
                {
                    ...
                    if (!Guid.TryParse(tenantId, out tempGuid))
                    {
                        ...
                        var homeTenants = tenants.Where(t => t.IsHome).ToList();
                        tenants = homeTenants ?? tenants;
                ...
                else
                {
                    ...
                    foreach (var tenant in tenants)
                    {
                        ...
                        if(token != null)
                        {
                            ...
                            {
                                if (tempSubscription.GetRetrievedByTenant() == tempSubscription.GetHomeTenant())
                                {
                    ...
                    newTenant = new AzureTenant { Id = newSubscription?.GetRetrievedByTenant() };
                }
            }
            ...
            if (newSubscription == null)
            {
                ...
                //fixme: what about both tenant and subscription are null?
            ...
            if (shouldPopulateContextList)
            {
                ...
                {
                    var tenantIdFromSubscription = subscription.GetRetrievedByTenant();
        ...
        public IAzureContext SetCurrentContext(string subscriptionNameOrId, string tenantId, string name = null)
        {
            ...
            if (!string.IsNullOrWhiteSpace(subscriptionNameOrId))
            {
                ...
                var tenantFromSubscription = subscription.GetRetrievedByTenant();
        ...
        //{
        //    //fixme: why use list instead of get
        ...
        //        subscription = clientSwitcher?.GetSubscriptionById(subscriptionId, accessToken, account, environment);
            ...
            out IAzureSubscription subscription)
        {
            ...
            if (null != accessToken)
            {
                try
                {
                    ...
                    {
                        subscription = clientSwitcher?.GetSubscriptionById(subscriptionId, accessToken, new AzureAccount { Id = accessToken.UserId }, environment);
                        found = subscription != null;

                    }
                    else
                    {
                        var subscriptions = clientSwitcher?.ListAllSubscriptionsForTenant(accessToken, new AzureAccount { Id = accessToken.UserId }, environment)?.ToList()
                        ...
                        account.SetSubscriptions(subscriptions.Select(i => i.GetId().ToString()).ToArray());
			...
			Action<string> promptAction)
        {
            ...
            try
            {
                ...
                result = clientSwitcher?.ListAccountTenants(commonTenantToken, environment);
                //fixme:side-effects?to enable?
                //account.SetProperty(AzureAccount.Property.Tenants, result.Select(t => t.Id).ToArray());
            }
            catch
            {
            ...
            string tenantId)
        {
            ...
            return clientSwitcher?.ListAllSubscriptionsForTenant(accessToken, account, environment);
        }
        ...
        private ISubscriptionClientSwitcher clientSwitcher;
    }
}
