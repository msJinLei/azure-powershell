...
namespace Microsoft.Azure.Commands.Profile
{
    ...
    public class ConnectAzureRmAccountCommand : AzureContextModificationCmdlet, IModuleAssemblyInitializer
    {
        ...
        [Parameter(ParameterSetName = ServicePrincipalCertificateFileParameterSet, Mandatory = false, HelpMessage = "Specifies if the x5c claim (public key of the certificate) should be sent to the STS to achieve easy certificate rollover in Azure AD.")]
        public SwitchParameter SendCertificateChain { get; set; }


        [Parameter(ParameterSetName = ServicePrincipalCertificateFileParameterSet, Mandatory = true, HelpMessage = "The path of certficate file in pem format.")]
        public String CertificatePath { get; set; }

        [Parameter(ParameterSetName = ServicePrincipalCertificateFileParameterSet, HelpMessage = "The password required to access the X.509 certificate data.")]
        ...
        public override void ExecuteCmdlet()
        {
            ...
            {
                //fixme
                azureAccount.SetProperty("CertificatePath", CertificatePath);
                if (this.IsBound(nameof(CertificatePassword)))
                {
                    azureAccount.SetProperty("CertificatePassword", CertificatePassword.ConvertToString());
                }
            }
