            ...
            string resourceId = AzureEnvironment.Endpoint.ActiveDirectoryServiceEndpointResourceId)
        {
            switch (account.Type)
            {
                ...
                case AzureAccount.AccountType.ServicePrincipal:
                    ...
                    var certificatePassword = ConvertToSecureString(account.GetProperty("CertificatePassword"));
                    return new ServicePrincipalParameters(tokenCacheProvider, environment, tokenCache, tenant, resourceId, account.Id, account.GetProperty(AzureAccount.Property.CertificateThumbprint), account.GetProperty("CertificatePath"),
