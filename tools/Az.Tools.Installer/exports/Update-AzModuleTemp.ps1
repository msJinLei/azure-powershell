...
#>
    ...
    process {
        ...
        $key = 'Az.Accounts'
        if ($installModules.ContainsKey($key)) {
            if ($Force -or $PSCmdlet.ShouldProcess("Update $key to the latest version from $($($installModules[$key][0]).Item0)"
            , $key, "Update to the latest version")) {
                PowerShellGet\Update-Module -Name $key
            }
        }

        $key = $null
        foreach ($key in $installModules.Keys) {
            if ($key -ne 'Az.Accounts' -and $key -ne 'Az') {
                if ($Force -or $PSCmdlet.ShouldProcess("Update $key to the latest version from $($($installModules[$key][0]).Item0)"
                , $key, "Update to the latest version")) {
                    PowerShellGet\Update-Module -Name $key
                }
        ...
        $key = $null
        foreach ($key in $installModules.Keys) {
            $installedVersions = PowerShellGet\Get-InstalledModule -Name $key -AllVersions

            foreach($version in $installedVersions) {
                if ($version.Repository -ne $installModules[$key][0].Item0) {
                    PowerShellGet\Uninstall-Module -Name $key -RequiredVersion $version.Version
                }
                else {
                    break
                }
            ...
            if($RemovePrevious) {
                $tuples = $installedModules[$key]
                if ($Force -or $PSCmdlet.ShouldProcess("Remove $key of the versions: $($tuples.Foreach({$_.Item1}) -Join ',')"
                ,$key ,"Remove all previous versions of")) {
                    foreach($tuple in $tuples) {
                        PowerShellGet\Uninstall-Module -Name $key -RequiredVersion $tuple.Item1
                    }
                }
            }
        }
