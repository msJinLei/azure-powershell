...
$items = Get-ChildItem -Recurse -Directory -Path $Repo -Filter 'help'
foreach ($item in $items)
{
    $helpFolder = $item.FullName
    Write-Verbose "Remove ProgressAction for $helpFolder"
    $markdowns = Get-ChildItem -Recurse -File -Include "*.md" -Path $helpFolder
    Repair-PlatyPSMarkdown -Path $markdowns -ParameterName @('ProgressAction')
}
